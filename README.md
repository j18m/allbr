...
### 1. 单个IP端口探测
```bash
./allbr port -t 192.168.1.1 -p 80 -T
```

### 2. 多个端口探测
```bash
./allbr port -t 192.168.1.1 -p 80,443,8080 -T
```

### 3. 保存结果到CSV文件
```bash
./allbr port -t 192.168.1.1 -p 80 -T -O port_scan.csv
```

### 4. 文件输入模式
```bash
./allbr port -t file://test_ips.txt -p 80,443 -T -O port_results.csv
```
...
|------|------|------|--------|
| --targets | -t | 目标IP、IP段或文件 | - |
| --port | -p | 要探测的端口，多个用逗号分隔 | - |
| --threads | -n | 线程数 | 10 |
| --timeout | -o | 超时时间（秒） | 5 |
| --title | -T | 获取网页标题信息 | false |
| --output | -O | 结果输出文件，支持`.txt`和`.csv`格式 | - |
